...
//reduce method takes in a parameter of a callback function, it can take in up to 4 elements, the return value is the accumulated result, and the 
// parameters: previousValue, currentValue, currentIndex, initialValue 
//must set initialValue so the method will start at 0 versus the first element value given

let initialValue = 0;

let yearsLived = presidents.reduce((accumulator, currentValue) => {
    //we create a local scope sum variable 
    let sum;
        //use . to dig into array for element value
        //if statement to pass over each pres in array
        //checking live presidents for 2020 - birth:
        //must set undefined to 2020 to check age of presidents still alive
    if(currentValue.passsed === undefined){
        sum = accumulator + 2020 - currentValue.year
    } else {
        //checking passed pres for passed - year:
