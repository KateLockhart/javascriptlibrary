...
//my attempt below
//presidents.forEach(info => console.log(info)) 

//solution
//denote an array with [] and object with {} with push/let copy variable to indicate what kind of element we're creating
// new function crated per first step of instruction 
let fn = () => {
    // creating local scope variable to become new array, denote the array with [], now can call on/add to it 
    let copy = [];
    //using .forEach to call on presidents array above
    presidents.forEach(presidents => {
        // .push method calls on/modifies created array copy
        copy.push({
            // used to create values in the copy array, adds them as objects because of {}
            // dot notation used to pull info from presidents const
            last: presidents.last,
            yearBorn: presidents.year
        })
    })
    // returns copy array created
    return copy;
}
//new variable is created to call on new function
let newPresArray = fn();
